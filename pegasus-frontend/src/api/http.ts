/** Shared HTTP helpers: API base URL, error classification, fetch wrapper. */

const apiBase = (import.meta as any).env.VITE_API_BASE ?? ''

export function absoluteApiUrl(pathOrUrl: string | undefined): string | undefined {
 ...
 */
export function classifyHttpFailure({ error, response, rawBody }: HttpFailureCtx): {
  kind: 'backend_down' | 'http' | 'parse' | 'unknown'
  message: string
} {
...
export function messageFromHttpFailure(ctx: HttpFailureCtx, fallback = 'Request failed'): string {
...
export async function checkBackendHealth(): Promise<{ ok: boolean; payload?: any; error?: string }> {
  try {
