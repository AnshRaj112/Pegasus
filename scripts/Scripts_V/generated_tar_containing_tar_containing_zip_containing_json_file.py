...
# Case 14
df_src.write_ndjson("src.json")
df_tgt.write_ndjson("tgt.json")

for prefix in ["src", "tgt"]:
    with zipfile.ZipFile(f"inner_{prefix}.zip", "w") as z:
        z.write(f"{prefix}.json")
    with tarfile.open(f"inner_{prefix}.tar", "w") as tar:
        tar.add(f"inner_{prefix}.zip")
    with tarfile.open(f"case14_{prefix}.tar", "w") as tar:
        tar.add(f"inner_{prefix}.tar")

cleanup_temps(["src.json", "tgt.json", "inner_src.zip", "inner_tgt.zip", "inner_src.tar", "inner_tgt.tar"])
print("Case 14 generated.")