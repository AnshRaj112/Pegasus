...
for p in ["src", "tgt"]:
    ...
    with tarfile.open(f"inner_{p}.tar", "w") as tar: tar.add(f"inner_{p}.zip")
    with tarfile.open(f"case10_{p}.tar", "w") as tar: tar.add(f"inner_{p}.tar")
cleanup_temps(["src.avro", "tgt.avro", "inner_src.zip", "inner_tgt.zip", "inner_src.tar", "inner_tgt.tar"])
print("Case 10 generated.")